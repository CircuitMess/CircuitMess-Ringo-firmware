...
uint8_t heldF; // Robert v2 - press to number
	...
	while(1)
	{
		...
		else if(!played)
		{
			...
			if(ringVolume == 0)
				...
				ringtone->setVolume(ringVolume*21);
...
String MAKERphone::textInput(String buffer, int16_t length)
{
	...
	if (buttons.released(BTN_LEFT) && buffer != "")
	{
		textPointer--;
		 //buffer.
	}
	else if (buttons.released(BTN_RIGHT) && buffer != "")
	{
		if (textPointer != buffer.length())
			textPointer++;
		 //buffer.
	}
	else if (buttons.released(BTN_FUN_LEFT) && buffer != "")
	{
		if (textPointer == buffer.length())
			textPointer--;
	...
	for(int pomF = 0; pomF <= 8; pomF++ ){ // Holding a certain button down prints out a number
		if(buttons.held(pomF,30)) // Robert v2
	{
		textPointer++;
		buffer+=(String)(pomF+1);
		heldF = pomF+1;	// Robert
		while(buttons.pressed(pomF)); 
	...
	if(buttons.held(BTN_0,30)){
		textPointer++;
		buffer+='+';
		heldF = 10;	// Robert
		while(buttons.pressed(BTN_0)); 
			if (buffer.length() == length) {
		textLimitFlag = 1;
		return buffer;
		} 
	...
	if((length == -1 || length >= buffer.length()) && !heldF){ // Robert v2 dodao HeldF
		...
		else if (ret) // We don't have a key but the user is still cycling through characters on one key so we need to update the screen
		{
 		...
 		for(int pomF = 0; pomF <= 8; pomF++){
			if (heldF ==  pomF+1 && buttons.released(pomF)) {
				heldF = 0;
					}
			}

	if(heldF == 10 && buttons.released(BTN_0)) {
					heldF = 0;
	} 
	...
	for(int i = 0; i < 4;i++)
	{
		if(tracks[i] != nullptr)
		{
			if(mediaVolume == 0)
				...
				tracks[i]->setVolume(mediaVolume*21);
		}
		if(oscs[i] != nullptr)
			...
			switch (cursor)
			{
				case 0: //volume
					while(!buttons.released(BTN_B) && !buttons.released(BTN_A))
					{
						...
						if(buttons.repeat(BTN_LEFT,5)&& mediaVolume >0){
							{
							mediaVolume--;
							for(uint8_t i = 0 ; i < 4;i++)
							{
						...
						if(buttons.repeat(BTN_RIGHT,5)&& mediaVolume < 14)
						{
							mediaVolume++;
							for(uint8_t i = 0 ; i < 4;i++)
							{
