...
String MAKERphone::textInput(String buffer, int16_t length)
{
 //	heldF = 0;
	...
	else if (buttons.held((BTN_FUN_LEFT),50) && buffer != ""){
		int pp = buffer.length();
		for(int i = 0; i < pp; i++)
		buffer.remove(pp-i-1);
	}
	else if(key == '*') 
	{
		if (textPointer != buffer.length())
...
/* 		else if(getJoystickX() <100 ) // Robert -> pokušaj micanja v2
	{
		if (textPointer != buffer.length())
			textPointer++;
	}
			else if(getJoystickX() > 1000 ) // Robert -> pokušaj micanja v2
	{
		if (textPointer != 0)
			textPointer--;
	} */
