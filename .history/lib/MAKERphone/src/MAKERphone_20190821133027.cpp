...
uint8_t heldF = 0; // Robert v2 - press to number
...
/* 		else if(getJoystickX() <100 ) // Robert -> pokušaj micanja v2
	...
	if((length == -1 || length >= buffer.length()) && !heldF){ // Robert v2 dodao HeldF
		...
		for(int pomF = 0; pomF <= 8; pomF++){
		if (heldF ==  pomF+1 && !buttons.pressed(pomF)) heldF = 0;
			}

	if(heldF == 10 && !buttons.pressed(BTN_0)) heldF = 0;
