	...
	while(1)
	{
		...
		else if(!played)
		{
			...
			if(ringVolume == 0)
				...
				ringtone->setVolume(map(ringVolume, 0, 14, 100, 200));
	...
	for(int i = 0; i < 4;i++)
	{
		if(tracks[i] != nullptr)
		{
			if(mediaVolume == 0)
				...
				tracks[i]->setVolume(map(ringVolume, 0, 14, 100, 200));
		}
		if(oscs[i] != nullptr)
			...
			switch (cursor)
			{
				case 0: //volume
					while(!buttons.released(BTN_B) && !buttons.released(BTN_A))
					{
						...
						if(buttons.released(BTN_LEFT) && mediaVolume > )
						{
							mediaVolume--;
							for(uint8_t i = 0 ; i < 4;i++)
							{
						...
						if(buttons.repeat(BTN_RIGHT,5)&& mediaVolume < 10)
						{
							mediaVolume++;
							for(uint8_t i = 0 ; i < 4;i++)
							{
						...
						if(buttons.released(BTN_RIGHT) && mediaVolume < 10)
						{
							mediaVolume++;
							for(uint8_t i = 0 ; i < 4;i++)
							{
